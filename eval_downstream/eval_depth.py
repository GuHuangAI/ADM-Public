            ...
            for idx, batch in tqdm(enumerate(self.dl)):
                for key in batch.keys():
                    if isinstance(batch[key], torch.Tensor):
                        batch[key].to(device)
                image = batch["image"]
                image = unnormalize_to_zero_to_one(image) #* 10  # depth [0, 10]
                cond = batch['cond']
                ...
                for j, (img, c) in enumerate(zip(batch_pred, cond)):
                    ...
                    #img = 10 * img # [0, 10]
                    img[img < min_depth_eval] = min_depth_eval
                    img[img > max_depth_eval] = max_depth_eval
                    img[torch.isinf(img)] = max_depth_eval
                    gt_depth = image[j]
                    gt_depth[torch.isinf(gt_depth)] = 0
                    gt_depth[torch.isnan(gt_depth)] = 0
                    valid_mask = torch.logical_and(gt_depth > min_depth_eval, gt_depth < max_depth_eval)
                    silog, log10, abs_rel, sq_rel, rms, log_rms, d1, d2, d3 = compute_errors(
                        gt_depth[valid_mask], img[valid_mask])
                    silogs.append(silog)
                    log10s.append(log10)
                    abs_rels.append(abs_rel)
                    sq_rels.append(sq_rel)
                    rmss.append(rms)
                    log_rmss.append(log_rms)
                    d1s.append(d1)
                    d2s.append(d2)
                    d3s.append(d3)
                    num += 1
                    ...
                    img_depth = img # * 1000.0
                    # img_depth = img_depth.squeeze(0).detach().cpu().numpy().astype(np.uint16)
                    # cv2.imwrite(str(file_name)[:-4] + ".png", img_depth)
                    tv.utils.save_image(img, str(file_name)[:-4] + ".png")
            results_dict['silog'] = sum(silogs) / len(silogs)
            results_dict['log10'] = sum(log10s) / len(log10s)
            results_dict['abs_rel'] = sum(abs_rels) / len(abs_rels)
            results_dict['sq_rel'] = sum(sq_rels) / len(sq_rels)
            results_dict['rms'] = sum(rmss) / len(rmss)
            results_dict['log_rms'] = sum(log_rmss) / len(log_rmss)
            results_dict['d1'] = sum(d1s) / len(d1s)
            results_dict['d2'] = sum(d2s) / len(d2s)
            results_dict['d3'] = sum(d3s) / len(d3s)
