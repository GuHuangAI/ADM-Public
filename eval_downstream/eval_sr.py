        ...
        with torch.no_grad():
            ...
            for idx, batch in tqdm(enumerate(self.dl)):
                ...
                for j, (img, c) in enumerate(zip(batch_pred, cond)):
                    ...
                    c_min = c.min()
                    if c_min < 0:
                        c = unnormalize_to_zero_to_one(c)
                    else:
                        c = c[0].detach().cpu().numpy() * 19
                        c = c.astype(np.uint8)
                        c_color = np.zeros((c.shape[-2], c.shape[-1], 3))
                        for pi in range(0, 20):
                            index = np.where(c == pi)
                            c_color[index[0], index[1], :] = part_colors[pi]
                        c = torch.from_numpy(c_color.astype(np.float32)).permute(2, 0, 1)/255.
