...
from unet import utils, layers, layerspp, normalization
...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    if progressive != 'output_skip':
      modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                  num_channels=in_ch, eps=1e-6))
      modules.append(conv3x3(in_ch, channels*config.out_mul, init_scale=init_scale))
  ...
  def forward(self, x, time_cond):
    ...
    if self.config.scale_by_sigma:
      used_sigmas = used_sigmas.reshape((x.shape[0], *([1] * len(x.shape[1:]))))
      used_sigmas = used_sigmas.repeat(1, 3, 1, 1)
      used_sigmas_ = 1.001 - used_sigmas
      if self.out_mul > 2:
        used_sigmas = used_sigmas.repeat(1, self.out_mul-1, 1, 1)
      used_sigmas = torch.cat([torch.exp(-used_sigmas), torch.exp(-used_sigmas_)], dim=1)

      # used_sigmas = self.sigma_layer(temb)
      # used_sigmas = used_sigmas.reshape((used_sigmas.shape[0], used_sigmas.shape[1], *([1] * len(x.shape[2:]))))
      # h = h/(1+used_sigmas)
      # used_sigmas1 = used_sigmas[:, 0].reshape((x.shape[0], *([1] * len(x.shape[1:]))))
      # h[:, :3] = h[:, :3] / used_sigmas1
      # used_sigmas2 = used_sigmas[:, 1].reshape((x.shape[0], *([1] * len(x.shape[1:]))))
      # h[:, 3:] = h[:, 3:] / used_sigmas2
      h = h / used_sigmas
