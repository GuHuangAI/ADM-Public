...
class Unet(nn.Module):
    ...
    def forward(self, x, time, cond, text_context, x_self_cond = None, sigma_max=1, *args, **kwargs):
        ...
        c_out1 = t / (t + 1).sqrt()
        c_out2 = torch.sqrt(1 - t) / (1 + t).sqrt()
        c_in = 1
        c_noise = time.log() / 2
        ...
        # hm = []
        # hm2 = []
        for i, ((block1, block2, attn, downsample), relation_layer, relation_layer_text) \
                in enumerate(zip(self.downs, self.relation_layers_down, self.relation_layers_text_down)):
            x = relation_layer_text(x, text_context)
            x = block1(x, t)
            h.append(x)
            h2.append(x.clone())
            # m = m_block(m, t)
        ...
        x = x1
        for (block1, block2, attn, upsample), relation_layer, relation_layer_text in \
            zip(self.ups, self.relation_layers_up, self.relation_layers_text_up):
            x = relation_layer_text(x, text_context)
        ...
        x = x2
        for (block1, block2, attn, upsample), relation_layer, relation_layer_text in \
                zip(self.ups2, self.relation_layers_up2, self.relation_layers_text_up2):
            x = relation_layer_text(x, text_context)
