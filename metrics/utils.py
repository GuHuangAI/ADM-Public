...
from metrics.datasets import ImagesPathDataset
from metrics.defaults import DEFAULTS
from metrics.feature_extractor_base import FeatureExtractorBase
from metrics.generative_model_base import GenerativeModelBase
# from metrics.generative_model_onnx import GenerativeModelONNX
from metrics.helpers import get_kwarg, vassert, vprint
from metrics.registry import DATASETS_REGISTRY, FEATURE_EXTRACTORS_REGISTRY, SAMPLE_SIMILARITY_REGISTRY, \
...
def create_feature_extractor(name, list_features, cuda=True, **kwargs):
    ...
    feat_extractor = cls(name, list_features, **kwargs)#.to(torch.float32)
    feat_extractor.eval()
    if cuda:
        feat_extractor.cuda()
