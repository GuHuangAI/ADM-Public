...
class DDPM(nn.Module):
    ...
    @torch.no_grad()
    def sample_fn_2order(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps * 2
        alpha = 1
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps // 2)
        if self.sampling_timesteps % 2 != 0:
            time_steps = torch.cat((time_steps, torch.tensor([1. - self.sampling_timesteps // 2 * step], device=device)), dim=0)
        if denoise:
            eps = self.eps
            time_steps = torch.cat((time_steps[:-1], torch.tensor([time_steps[-1] - eps], device=device), \
                                    torch.tensor([eps], device=device)), dim=0)
        ...
        for i, time_step in enumerate(time_steps):
            ...
            if i == time_steps.shape[0] - 1:
                s = cur_time
            sigma = torch.sqrt(s * (cur_time - s) / cur_time)
            if cond is not None:
                pred = self.model(img, cur_time, cond)
            else:
                pred = self.model(img, cur_time)
            ...
            C = -1 * x0
            # Euler step.
            d_img = - C - 1 / torch.sqrt(cur_time) * noise
            img_h = img + h * d_img
            cur_time_h = cur_time - h
            # Apply 2-order correction.
            if s >= step:
                if cond is not None:
                    pred = self.model(img_h, cur_time_h, cond)
                else:
                    pred = self.model(img_h, cur_time_h)
                C, noise = pred[:2]
                # correct C
                x0 = self.pred_x0_from_xt(img_h, noise, C, cur_time_h)
                if self.clip_x_start:
                    x0.clamp_(-1., 1.)
                C = -1 * x0
                d_prime = - C - 1 / torch.sqrt(cur_time_h) * noise
                img_h = img + h * ((1 - 1 / 2 / alpha) * d_img + 1 / 2 / alpha * d_prime)
            # Add stochastic sampling.
            epsilon = torch.randn_like(img, device=img.device)
            img = img_h + sigma * epsilon
            cur_time = cur_time_h
        img.clamp_(-1., 1.)
        if unnormalize:
            img = unnormalize_to_zero_to_one(img)
